...
Stack: sentence-transformers (embedding lokal) + Ollama Qwen2.5 (LLM lokal).

Cara Pakai:
  python main.py
  python main.py --role frontend
  python main.py --role fullstack --duration 1
"""
...
def main():
    parser = argparse.ArgumentParser(description="VerifyLearn - Live Test AI Engine Runner (Local Stack)")
    ...
    parser.add_argument("--duration", type=int, default=2, help="Durasi belajar dalam bulan")
    parser.add_argument("--ollama-host", default="http://localhost:11434", help="URL Ollama server")
    args = parser.parse_args()

    print("============================================================")
    print("      VerifyLearn — Pengujian Sistem Live AI (Local Stack)   ")
    print("      Embedding: sentence-transformers (all-MiniLM-L6-v2)    ")
    print("      LLM      : Qwen2.5 via Ollama                          ")
    print("============================================================")
    ...
    if vector_store.count() == 0:
        print("\n⚠️  PERINGATAN: ChromaDB kosong! Jalankan ingest terlebih dahulu:")
        print('   python build_knowledge_base.py --roadmap-dir "..." --output-dir data/knowledge_base --ingest --api-key "dummy"')
        return
    ...
    try:
        ...
        plan = engine.generate_learning_plan(role=args.role, duration_months=args.duration)
        ...
        # Ambil materi pertama dari plan untuk test fungsi-fungsi LLM berikutnya
        first_material = plan.weekly_schedule[0]["materials"][0]
        print(f"\n📌 Materi yang dipakai untuk test LLM berikutnya: '{first_material['title']}'")

        # ── TEST 2: GENERATE QUIZ ────────────────────────────────────────────
        print_divider("Generate Quiz (RAG-Driven)")
        quiz_questions = engine.generate_quiz(material=first_material, n_pg=2, n_essay=1)

        print(f"✅ Berhasil membuat {len(quiz_questions)} soal kuis untuk topik '{first_material['title']}'")
        ...
        challenge = engine.generate_livecode_challenge(material=first_material, difficulty="normal")
        ...
        trigger_reason = "User melakukan paste kode dalam jumlah besar dalam waktu 2 detik (indikasi copy-paste/AI)"
        ...
        final_challenge = engine.generate_final_challenge(material=first_material)
    ...
    print("\n" + "="*60)
    print(" 🎉 SELESAI! Seluruh pipeline RAGEngine teruji.")
    print("="*60)
